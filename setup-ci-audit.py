...
def run_gh_query(date_list:list[str], orgs:list[str], use_init_date:bool=False, no_dates_on_repo:bool=False):
    ...
    with open("audit_setup.sh","w") as shell:
        for org in orgs:
            if no_dates_on_repo:
              command = command_template.replace("[ORG_NAME]",org)
              shell.write(command + "\n")
              shell.write("sleep 2\n")
            else:
...
def parse_args() -> tuple[str,str,int,list[str],bool]:
    ...
    if not check_quarter and not args.no_dates_flag:
    ...
    return start, end, quarter, org_names, args.use_init, args.no_dates_flag
...
def main():
    ...
    """
    try:
        start, end, quarter, org_names, use_init, no_dates_flag = parse_args()
        print("Start date:", start, "\nEnd date:", end, "\nQuarter:", quarter, "\nOrgs:", org_names, "\nUse Initial Date:", use_init)
        use_date_range:bool = quarter not in [1,2,3,4]
        date_list:list[str] = []
        if not no_dates_flag:
          date_list = get_date_list(start=start,end=end,quarter=quarter,use_range=use_date_range)
        print("Running GitHub query")
        run_gh_query(date_list=date_list, orgs=org_names, use_init_date=use_init, no_dates_on_repo=no_dates_flag)
